...
- **AWS prod serving (Phase 2c, `infra/`)**: CDK Python app (`aws-cdk-lib`, isolated `infra/.venv`; excluded from root pytest via `--ignore=infra`, tested with `cd infra && .venv/bin/python -m pytest`). `PepOracleCertStack` (us-east-1: Route 53 zone + ACM cert) + `PepOracleProdStack` (ap-southeast-2: KMS, S3 corpus bucket, DynamoDB OAuth table matching `oauth_store.DynamoDbStore`, one-user Cognito pool, container Lambda `pep_oracle.server.handler` behind CloudFront+OAC over a Function URL, least-privilege IAM). Lambda env encodes the runtime contract (`SERVE_FROM_ARTIFACT=1`, `EMBED_BACKEND=bedrock`, `OAUTH_STORE=dynamodb`, `OAUTH_SIGNING_BACKEND=ssm`, `AUTHORIZE_GATE=cognito`). Authored only — deploy/cutover (incl. the direct `pep-oracle.iicapn.com` cutover) is the gated runbook `docs/aws/phase2c-deploy-runbook.md`.
