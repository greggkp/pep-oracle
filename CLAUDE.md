...
- **AWS prod serving (Phase 2c, `infra/`)**: CDK Python app (`aws-cdk-lib`, isolated `infra/.venv`; excluded from root pytest via `--ignore=infra`, tested with `cd infra && .venv/bin/python -m pytest`). `PepOracleCertStack` (us-east-1: Route 53 zone + ACM cert) + `PepOracleProdStack` (ap-southeast-2: KMS, S3 corpus bucket, DynamoDB OAuth table matching `oauth_store.DynamoDbStore`, one-user Cognito pool, container Lambda `pep_oracle.server.handler` fronted by CloudFront → **API Gateway HTTP API** ($default proxy), least-privilege IAM). Lambda env encodes the runtime contract (`SERVE_FROM_ARTIFACT=1`, `EMBED_BACKEND=bedrock`, `OAUTH_STORE=dynamodb`, `OAUTH_SIGNING_BACKEND=ssm`, `AUTHORIZE_GATE=cognito`). **Deployed to AWS 2026-06-07 but PAUSED**: infra is live + correct, but the app isn't Lambda-ready — MCP `StreamableHTTPSessionManager` vs Mangum per-invoke lifespan (all routes 500), read-only-FS `ensure_dirs`/ChromaDB on `/status`, 10s init timeout. `pep-oracle.iicapn.com` is NOT cut over (still on the tunnel). Deploy-discovered infra facts: account Lambda concurrency=10 (reserved-concurrency made optional), public Function URLs blocked account-wide (hence APIGW, not OAC+Function-URL). See `docs/aws/phase2c-lambda-compat-findings.md`; deploy/cutover runbook `docs/aws/phase2c-deploy-runbook.md`.
