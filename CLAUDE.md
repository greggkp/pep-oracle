...
- **Cold-path timing** (`timing.py`): `timed(phase, **fields)` logs one structured `timing phase=<name> ms=<n>` line per phase to the `pep_oracle.timing` logger. The MCP search path is instrumented (`search.*`, `embed.client_init`, `corpus.*`, `hybrid.bm25_build`) so a cold/first request's breakdown — S3 download, parquet parse incl. the `to_pylist()` embedding materialization, BM25 build, Bedrock embed — is readable in CloudWatch (Lambda's own `Init Duration` covers the import/INIT phase). Corpus load is **lazy on first request**, so the first `/mcp` search pays the corpus + index build; those phases also recur on each TTL refresh. See `docs/aws/cold-path-measurement.md`.
...
`deploy/` contains systemd units for the OptiPlex fallback box:
- `pep-oracle-api.service` — runs the MCP server (FastAPI; no web GUI). Kept as a DNS-rollback fallback after the AWS cutover.
- `pep-oracle-ingest.service` + `pep-oracle-ingest.timer` — periodic ingestion (disabled after the Phase 3 cutover; the Fargate job is authoritative).

**AWS prod (`infra/`, CDK Python; isolated `infra/.venv`, excluded from root pytest via `--ignore=infra`, tested with `cd infra && .venv/bin/python -m pytest`):**
...
- **CI/CD (Phase 4, `.github/workflows/`)**: `ci.yml` gates every PR + push to main (`ruff check` + root `pytest` + infra `pytest` + `cdk synth '*'` + docker build of both images, no AWS access). `deploy.yml` runs on a `v*` tag push, OR via `workflow_dispatch` with a `version` input (e.g. `v1.2.0`) so a release can be cut **without a local machine** — the dispatch path resolves `semver` from the input (not the branch ref), deploys, smoke-tests, then on success tags the deployed commit and pushes the tag with `GITHUB_TOKEN` (which deliberately does NOT re-trigger the `push: tags` event, so no double deploy; needs `contents: write`). It assumes the GitHub-OIDC deploy role from `PepOracleCicdStack`, runs `cdk deploy PepOracleProdStack PepOracleIngestStack` (deploy context via `env:`, `-c git_sha -c semver`), then `scripts/smoke.py` (auth-free: `/health`, `/version` sha+semver match, `/.well-known/...`, `/mcp` no-token→401, retries for cold start). The release tag surfaces as `GET /version` `code_semver` (`config.SEMVER` overrides the package version). Rollback = re-run `deploy` on the prior `v*` tag (image already in ECR); a re-dispatch of an existing version re-deploys and skips re-tagging. One-time bootstrap: `cdk deploy PepOracleCicdStack` with admin creds + set repo variables `AWS_DEPLOY_ROLE_ARN` + `ALLOWED_EMAIL`. `PepOracleCertStack` stays a manual deploy. Runbook `docs/aws/phase4-cicd-runbook.md`.
...
- A `PreToolUse` hook (`.claude/hooks/pre-commit.sh`) runs before `git commit` and blocks unless (1) `pytest -x -q` passes and (2) `/claude-md-improver` has been run with `.claude/.md-reviewed` touched newer than `CLAUDE.md`. Stage CLAUDE.md changes before committing so they land in the same commit.
- A `PostToolUse` hook (`.claude/hooks/restart-server.sh`, OptiPlex-only) runs `sudo systemctl restart pep-oracle-api.service` after `git commit` so local code changes reach the running server (logs: `journalctl -u pep-oracle-api.service`). Harmless elsewhere.
