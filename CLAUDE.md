...
- **Speaker-name mapping**: pyannote over-segments this audio (16–30 micro-clusters/episode), but the largest clusters are the substantive speakers (Chas ~58%, Dave ~34%) and the tail is Lachie/music/fragments. Do NOT cap `max_speakers` — it merges Chas and Dave into one blob. Instead `diarize.assign_substantive_speakers` labels only clusters above `SUBSTANTIVE_SPEAKER_SHARE` (15%): loudest→Chas always, next→Dave (if title names him via `host_roster_from_title`) else Guest, tail→`None` (skipped, no `has_speaker_*`). Favors precision (unattributed) over mislabeling. No `identify-speakers`/profiles file needed (profiles still override). Re-process existing data with `pep-oracle remap-speakers` — rebuilds chunks from cached transcript+diarization, reuses stored embeddings by `chunk_id` (no re-embed), handles out-of-feed episodes via metadata reconstruction.
- **Full speaker coverage / Chas-vs-Dave precision** (future): top-2-cluster mapping leaves over-split host fragments unattributed (~10–40% on some episodes) and can swap Chas/Dave when a guest out-talks a host. Real fix is voice-embedding speaker ID (store real Chas/Dave voice profiles, cosine-match clusters) instead of speaking-time heuristics.
